...
class UnicodeCategorySpec extends ObjectBehavior
{
    ...
    {
        $this->beConstructedWith(new Char('a'));
    ...
    {
        foreach ([']', ')', '}', '⟩', '｣', '⟧', '〕'] as $char) {
            expect((new UnicodeCategory(new Char($char)))->isLike(new ClosePunctuation()))->toBe(true);
        }
    }

    public function it_is_a_close_punctuation_with_string_scalar_type()
    {
        foreach ([']', ')', '}', '⟩', '｣', '⟧', '〕'] as $char) {
            expect((UnicodeCategory::fromString($char))->isLike(new ClosePunctuation()))->toBe(true);
        }
    }

    public function it_is_not_a_close_punctuation()
    {
        $this->beConstructedWith(new Char('>'));
    ...
    public function it_is_a_connector_punctuation()
    {
        foreach (['_', '‿', '⁀', '⁔', '︳', '︴', '﹍', '﹎', '﹏', '＿'] as $char) {
            expect((new UnicodeCategory(new Char($char)))->isLike(new ConnectorPunctuation()))->toBe(true);
        }
    }
    ...
    public function it_is_a_control()
    {
        foreach (['	', PHP_EOL, "\u{0000}", "\u{0099}", "\u{009F}"] as $char) {
            expect((new UnicodeCategory(new Char($char)))->isLike(new Control()))->toBe(true);
        }
    }

    public function it_is_not_a_control()
    {
        $this->beConstructedWith(new Char("\u{00A0}"));
    ...
    {
        foreach (['$', '£', '￡', '€', '₯', '₻', '＄', '฿', '﷼', '￦'] as $char) {
            expect((new UnicodeCategory(new Char($char)))->isLike(new CurrencySymbol()))->toBe(true);
        }
    }
    ...
    {
        foreach (['-', '᐀', '‒', '⸗', '⸚', '⸻', '゠', '︱', '－'] as $char) {
            expect((new UnicodeCategory(new Char($char)))->isLike(new DashPunctuation()))->toBe(true);
        }
    }
    ...
    public function it_is_a_decimal_digit_number()
    {
        foreach (['0', '9', '𝟘', '𝟡', '𝟢', '𝟫', '𝟬', '𝟵', '𝟶', '𝟿', '꧐', '꧙', '꘠', '꘩', '༠', '༩'] as $char) {
            expect((new UnicodeCategory(new Char($char)))->isLike(new DecimalDigitNumber()))->toBe(true);
        }
    }

    public function it_is_not_a_decimal_digit_number()
    {
        $this->beConstructedWith(new Char('O'));
    ...
    public function it_is_an_enclosing_mark()
    {
        foreach (['҈', '҉', '⃝', '⃞', '⃟', '⃠', '⃢', '⃣', '⃤', '꙰', '꙱', '꙲', '᪾'] as $char) {
            expect((new UnicodeCategory(new Char($char)))->isLike(new EnclosingMark()))->toBe(true);
        }
    }
    ...
    {
        foreach (['»', '’', '”', '›', '⸃', '⸅', '⸊', '⸍', '⸝', '⸡'] as $char) {
            expect((new UnicodeCategory(new Char($char)))->isLike(new FinalQuotePunctuation()))->toBe(true);
        }
    }
    ...
    public function it_is_a_format()
    {
        foreach (['؀', '۝', "\u{00AD}", "\u{E007F}"] as $char) {
            expect((new UnicodeCategory(new Char($char)))->isLike(new Format()))->toBe(true);
        }
    }
    ...
    {
        foreach (['«', '‘', '‛', '“', '‟', '‹', '⸂', '⸄', '⸉', '⸌', '⸜', '⸠'] as $char) {
            expect((new UnicodeCategory(new Char($char)))->isLike(new InitialQuotePunctuation()))->toBe(true);
        }
    }
    ...
    public function it_is_a_lowercase_letter()
    {
        foreach (['a', 'z', 'µ', 'ß', 'ϣ', 'ȱ', 'ƕ', 'œ', 'ê', 'ｚ',] as $char) {
            expect((new UnicodeCategory(new Char($char)))->isLike(new LowercaseLetter()))->toBe(true);
        }
    }
    ...
    {
        foreach (['+', '𝟃', '=', '±', '×', '⅀', '∑', '∭', '≧', '⋙', '◁', '⟙', '⨊'] as $char) {
            expect((new UnicodeCategory(new Char($char)))->isLike(new MathSymbol()))->toBe(true);
        }
    }

    public function it_is_not_a_math_symbol()
    {
        $this->beConstructedWith(new Char('ß'));
    ...
    public function it_is_a_modifier_letter()
    {
        foreach (['ʰ', 'ﾟ', 'ꜝ', 'ꜛ', 'ꜜ', 'ꘌ', '々', 'ᶴ', 'ヾ'] as $char) {
            expect((new UnicodeCategory(new Char($char)))->isLike(new ModifierLetter()))->toBe(true);
        }
    }

    public function it_is_not_a_modifier_letter()
    {
        $this->beConstructedWith(new Char('𖿡'));
    ...
    {
        foreach (['^', '꞊', '˜', '˂', '˃', '˘', '˟', '˩'] as $char) {
            expect((new UnicodeCategory(new Char($char)))->isLike(new ModifierSymbol()))->toBe(true);
        }
    }
    ...
    public function it_is_a_non_spacing_mark()
    {
        foreach (['̀', "\u{E01EF}", '𝆋', '𝅩', '𑀸', '𐨁', 'ꙴ', '゙'] as $char) {
            expect((new UnicodeCategory(new Char($char)))->isLike(new NonSpacingMark()))->toBe(true);
        }
    }

    public function it_is_not_a_non_spacing_mark()
    {
        $this->beConstructedWith(new Char("\u{E01F0}"));
    ...
    {
        foreach (['(', '｢', '《', '⦕', '❰', '༺'] as $char) {
            expect((new UnicodeCategory(new Char($char)))->isLike(new OpenPunctuation()))->toBe(true);
        }
    }
    ...
    public function it_is_an_other_letter()
    {
        foreach (['ª', '鼖', '𒈰', '𒈜', 'ꁇ', 'ㇱ', 'ㆊ', 'ㅢ', 'ㄨ', 'ゾ', 'の'] as $char) {
            expect((new UnicodeCategory(new Char($char)))->isLike(new OtherLetter()))->toBe(true);
        }
    }
    ...
    {
        foreach (['²', '🄌', '㈠', '𑁚', '𐭘', '㊷', '➊', '➉', '⒈', '⒑'] as $char) {
            expect((new UnicodeCategory(new Char($char)))->isLike(new OtherNumber()))->toBe(true);
        }
    }

    public function it_is_not_an_other_number()
    {
        $this->beConstructedWith(new Char('1'));
    ...
    {
        foreach (['!', '𒑴', '𑁍', '@', '＠', '／', '%', '&', '︖', '〽', '⸿', '⸘', '⁐'] as $char) {
            expect((new UnicodeCategory(new Char($char)))->isLike(new OtherPunctuation()))->toBe(true);
        }
    }

    public function it_is_not_an_other_punctuation()
    {
        $this->beConstructedWith(new Char('ㄨ'));
    ...
    {
        foreach (['¦', '￼', '🉄', '🆐', '🆏', '⻘', '⺠'] as $char) {
            expect((new UnicodeCategory(new Char($char)))->isLike(new OtherSymbol()))->toBe(true);
        }
    }
    ...
    {
        $this->beConstructedWith(new Char("\u{2029}"));
    ...
    public function it_is_a_private_use()
    {
        foreach (["\u{E000}", "\u{F8FF}", "\u{F0000}", "\u{FFFFD}", "\u{100000}", "\u{10FFFD}"] as $char) {
            expect((new UnicodeCategory(new Char($char)))->isLike(new PrivateUse()))->toBe(true);
        }
    }

    public function it_is_not_a_private_use()
    {
        $this->beConstructedWith(new Char("\u{10FFFF}"));
    ...
    public function it_is_a_space_separator()
    {
        foreach (["\u{0020}", "\u{2000}", "\u{200A}", "\u{202F}", "\u{205F}", "\u{3000}"] as $char) {
            expect((new UnicodeCategory(new Char($char)))->isLike(new SpaceSeparator()))->toBe(true);
        }
    }

    public function it_is_not_a_space_separator()
    {
        $this->beConstructedWith(new Char("\u{203F}"));
    ...
    public function it_is_a_spacing_combining_mark()
    {
        foreach (['ः', '𝅲', '𝅦', 'ꦿ', '᳡', 'ៀ', 'ြ', 'ෞ', 'ൈ', 'ௌ'] as $char) {
            expect((new UnicodeCategory(new Char($char)))->isLike(new SpacingCombiningMark()))->toBe(true);
        }
    }

    public function it_is_not_a_spacing_combining_mark()
    {
        $this->beConstructedWith(new Char("\u{1D173}"));
    ...
    public function it_is_a_titlecase_letter()
    {
        foreach (['ǅ', 'ῼ', 'ᾈ', 'ǋ', 'ᾬ'] as $char) {
            expect((new UnicodeCategory(new Char($char)))->isLike(new TitlecaseLetter()))->toBe(true);
        }
    }

    public function it_is_not_a_titlecase_letter()
    {
        $this->beConstructedWith(new Char("\u{FFFD}"));
    ...
    public function it_is_an_uppercase_letter()
    {
        foreach (['A', 'Z', 'À', 'Ž', '𝜜', '𝟊'] as $char) {
            expect((new UnicodeCategory(new Char($char)))->isLike(new UppercaseLetter()))->toBe(true);
        }
    }
